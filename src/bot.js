...
function welcomeText() {
  return [
    ...
    "",
    "Создавайте с ИИ фотосессии, аватарки, новые фоны, ретушь и фото на документы.",
    "",
    "📸 В «Фотостудии» выберите шаблон, затем отправьте фото в чат. Можно использовать и последнее загруженное фото.",
    "✍️ В «Своём запросе» загрузите фото и напишите, что изменить: например, «убери фон», «сделай ретушь» или «измени причёску».",
    "💡 Перед первой загрузкой откройте «Как выбрать фото» в нижнем меню — так результат будет точнее.",
    "",
    "Продолжая, вы принимаете Пользовательское соглашение и Политику конфиденциальности — ссылки ниже."
  ].join("\n");
}
  ...
  async handleMessage(message) {
    ...
    if (photo || document) {
      ...
      if (awaitingCustomPhoto) {
        ...
        await this.telegram.sendMessage(
          chatId,
          "✅ Фото загружено.\n\nТеперь опишите, что нужно поменять. Чем конкретнее запрос, тем точнее результат.\n\nНапример: «Сделай волосы более объёмными и уложенными мягкими волнами. Лицо, одежду, позу и фон не меняй».\n\nМожно также написать: «Убери банку с газона, остальное на фото не меняй»."
        );
        return;
      }
  ...
  async handleCallback(callbackQuery) {
    ...
    if (data === "action:custom" || data === "action:custom_source") {
      ...
      if (!user.lastPhoto?.fileId) {
        ...
        await this.telegram.sendMessage(
          chatId,
          "📷 Загрузите фото, которое хотите изменить.\n\nЗатем опишите, что нужно поменять. Чем конкретнее запрос, тем точнее результат.\n\nНапример: «Сделай волосы более объёмными и уложенными мягкими волнами. Лицо, одежду, позу и фон не меняй».\n\nМожно также написать: «Убери банку с газона, остальное на фото не меняй»."
        );
        return;
      }
      await this.store.setPending(userId, "awaiting_custom_prompt");
      await this.telegram.sendMessage(
        chatId,
        "Опишите, что нужно поменять. Чем конкретнее запрос, тем точнее результат.\n\nНапример: «Сделай волосы более объёмными и уложенными мягкими волнами. Лицо, одежду, позу и фон не меняй».\n\nМожно также написать: «Убери банку с газона, остальное на фото не меняй»."
      );
      return;
    }
